...
namespace ExConfigExchange.Services
{
    ...
    {
        /// <summary>
        ///     Ersatz für den Fehlende <see cref="Type" /> basierten switch" /> statement für einfache
        ///     <see cref="Type" />.
        /// </summary>
        private static readonly Dictionary<Type, Func<string, MemberInfo, object, bool, IExConfigItem>> _basicTypeMapper =
        ...
        /// <summary>
        ///     Diese Methode ist der Rekursive Version vom <see cref="GetIExConfigItemFrom(string, Type, object, bool)" /> für die
        ///     Modellierung vom komplexe <see cref="Type" />s.
        /// </summary>
        /// <param name="displayKey">Der display key.</param>
        /// <param name="type">
        ///     Der zu modelierende komplexe <see cref="Type" /> (z.B.s.: class" />,
        ///     interface" />).
        /// </param>
        ...
        private IExConfigItem GetIExConfigItemFromComplexType(string displayKey, Type type, object defaultValue = null, bool readOnly = false, Type implementationSearchForType = null)
        {
            ...
            if (typeof(IEnumerable).IsAssignableFrom(type))
            {
                ...
                collection.ItemTemplate = _basicTypeMapper.ContainsKey(innerType) ? _basicTypeMapper[innerType]("Collection Item", innerType, null!, false) : GetIExConfigItemFromComplexType("Collection Item", innerType);
                if (defaultValue != null)
                {
                    foreach (var item in defaultValue as IEnumerable)
                    {
                        ...
                        collection.Value.Add(_basicTypeMapper.ContainsKey(item.GetType()) ? _basicTypeMapper[item.GetType()]("Collection Item", item.GetType(), item, false) : GetIExConfigItemFromComplexType("Collection Item", item.GetType(), item));
                    }
                }
            ...
            )
            {
                ...
                return GetIExConfigItemFromComplexType(displayKey, defaultValue.GetType(), defaultValue, readOnly);
            }
            ...
            )
            {
                ...
                foreach (var prop in type.GetProperties())
                {
                    ...
                    if (!prop.ShouldLeaveNull())
                    {
                        ...
                        var propReadOnly = !prop.CanWrite || prop.IsReadOnly();
                ...
                foreach (var field in type.GetFields())
                {
                    ...
                    if (!field.ShouldLeaveNull())
                    {
                        ...
                        var fieldReadOnly = field.IsInitOnly || field.IsLiteral || field.IsReadOnly();
        ...
        public IExConfigItem GetIExConfigItemFrom(string displayKey, Type type, object defaultValue = null, bool readOnly = false)
        {
            ...
            return GetIExConfigItemFromComplexType(displayKey, type, defaultValue, readOnly);
        }
