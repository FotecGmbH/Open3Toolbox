...
{
    /// <summary>
    ///     Represents the gateway for the sensors with serial communication
    /// </summary>
    public class SerialServerForSensors : IServerForSensors
    {
        /// <summary>
        ///     The locker
        /// </summary>
        ...
        public SerialServerForSensors(List<Sensor> sensors, object locker)
        {
        ...
        private void RefreshPorts()
        {
            lock (_locker)
            {
                ...
                foreach (var iP in initaInitializedPorts)
                {
                    CurrentPorts.Add(iP.Key, iP.Value);
                }
            }
        }
        ...
        private async Task GetDataFromSensorsWorkerAsync()
        {
            while (!Exit)
            {
                lock (_locker!)
                {
                    foreach (var (portName, currentSerialPort) in CurrentPorts)
                    {
                        byte[] data = Array.Empty<byte>();

                        try
                        {
                            if (currentSerialPort.BytesToRead > 1)
                            {
                                Thread.Sleep(500); //2000
                                lock (_locker)
                                {
                                    data = new byte[currentSerialPort.BytesToRead];
                                    currentSerialPort.Read(data, 0, currentSerialPort.BytesToRead);
                                }
                            }
                        }
                        catch (Exception)
                        {
                            Console.WriteLine("could not read from serial port: " + portName + " at " + DateTime.Now.ToLongTimeString());
                            // Serial Port is often not available for a short amount of time
                        }
